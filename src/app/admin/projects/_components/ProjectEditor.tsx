...
export default function ProjectEditor({ initialData, id }: ProjectEditorProps) {
  ...
  const clearDraft = () => {
    localStorage.removeItem(`medgenz-project-draft-${id || 'new'}`);
  };
  ...
  const processContentImages = async (html: string): Promise<string> => {
    ...
    for (let i = 0; i < images.length; i++) {
      if (images[i].src.startsWith('data:image/')) {
        try {
          images[i].src = await uploadImage(images[i].src);
  ...
  const updateHighlight = (index: number, val: string) => {
    const h = [...highlights]; h[index] = val; setHighlights(h);
  };
        ...
        <div className="lg:col-span-8 space-y-12">
          <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm space-y-8">
             <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-2">
                    ...
                    <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full text-xl font-bold text-slate-900 border-b-2 border-slate-50 focus:border-brand-600 outline-none p-2" required />
                </div>
                <div className="space-y-2">
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Subtitle</label>
                    <input type="text" value={subtitle} onChange={(e) => setSubtitle(e.target.value)} className="w-full text-xl font-bold text-slate-900 border-b-2 border-slate-50 focus:border-brand-600 outline-none p-2" />
                </div>
             </div>
             <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-2">
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Location</label>
                    <div className="flex items-center gap-2 border-b-2 border-slate-50 focus-within:border-brand-600 p-2"><MapPin className="w-4 h-4 text-slate-400" /><input type="text" value={location} onChange={(e) => setLocation(e.target.value)} className="w-full text-slate-900 outline-none" /></div>
                </div>
                <div className="space-y-2">
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Service</label>
                    <select value={service} onChange={(e) => setService(e.target.value)} className="w-full text-slate-900 border-b-2 border-slate-50 focus:border-brand-600 outline-none p-2 font-bold"><option>Modular OT</option><option>MGPS</option><option>Hospital Furniture</option><option>IVF Lab</option><option>Nurse Call</option></select>
                </div>
             </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-12">
             <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-6">
                <div className="flex items-center justify-between"><h3 className="text-lg font-bold text-slate-900 uppercase">Highlights</h3><button type="button" onClick={addHighlight} className="p-2 bg-brand-50 text-brand-600 rounded-lg hover:bg-brand-600 hover:text-white transition-all"><Plus className="w-4 h-4" /></button></div>
                <div className="space-y-3">{highlights.map((h, i) => (<div key={i} className="flex gap-2"><input type="text" value={h} onChange={(e) => updateHighlight(i, e.target.value)} placeholder="e.g. ISO Certified" className="w-full px-4 py-3 rounded-xl bg-slate-50 text-sm font-medium border-none outline-none focus:ring-2 focus:ring-brand-600/20" /><button type="button" onClick={() => removeHighlight(i)} className="p-3 text-slate-300 hover:text-red-500"><Trash2 className="w-4 h-4" /></button></div>))}</div>
             </div>
             <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-6">
                <div className="flex items-center justify-between"><h3 className="text-lg font-bold text-slate-900 uppercase">Specs</h3><button type="button" onClick={addSpec} className="p-2 bg-brand-50 text-brand-600 rounded-lg hover:bg-brand-600 hover:text-white transition-all"><Plus className="w-4 h-4" /></button></div>
                <div className="space-y-4">{specs.map((s, i) => (<div key={i} className="flex flex-col gap-2 p-4 bg-slate-50 rounded-2xl relative group"><input type="text" value={s.label} onChange={(e) => updateSpec(i, 'label', e.target.value)} placeholder="Label" className="bg-transparent text-[10px] font-black uppercase text-brand-600 outline-none" /><input type="text" value={s.value} onChange={(e) => updateSpec(i, 'value', e.target.value)} placeholder="Value" className="bg-transparent text-sm font-bold text-slate-900 outline-none" /><button type="button" onClick={() => removeSpec(i)} className="absolute top-2 right-2 p-1 text-slate-200 hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button></div>))}</div>
             </div>
          </div>
        </div>
