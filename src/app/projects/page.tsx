              ...
              {projects.map((p) => (
                <Link href={`/projects/${p.slug}`} key={p.id} className="group flex flex-col h-full bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500">
                  ...
                  <div className="p-10 flex-grow flex flex-col">
                    <h3 className="text-2xl font-black text-slate-900 mb-4 group-hover:text-brand-600 transition-colors uppercase tracking-tighter line-clamp-1 no-break">
                      {p.title}
                    </h3>
                    <p className="text-slate-500 leading-relaxed mb-8 flex-grow font-light line-clamp-3 no-break">
                      {stripHtml(p.brief)}
                    </p>
